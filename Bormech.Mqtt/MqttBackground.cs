...
public class MqttBackgroundService
    : BackgroundService
{
    private static string _topic = "bell/send/#";
    ...
    protected virtual async Task OnMyEvent(MyEventArgs message)
    {
    ...
    public  MqttBackgroundService(ILogger<MqttBackgroundService> logger)  
    {
        ...
        // _mqttRouter = mqttRouter;
        // _handlerFactory = new HandlerFactory();
        // _handlerFactory.RegisterHandler(_topic+"/temperature", () => new TemperatureHandler(this));
        ...
        if (!_mqttClient.IsConnected)
        {
            ...
            catch (Exception ex)
            {
                ...
                Task.Delay(TimeSpan.FromSeconds(5)); // Odczekaj przed ponowną próbą
            }
        }
    } 
